...
- **cm-super** — Type 1 versions of Computer Modern fonts. Without this,
  matplotlib usetex falls back to bitmap fonts and figures render poorly
  (jagged glyphs, broken PDF embedding).
- **dvipng** — converts DVI output (LaTeX's intermediate format) to PNG
  raster images. matplotlib drives LaTeX → DVI → PNG when `usetex=True`.
- **ghostscript** — needed for PostScript / EPS handling and font
  substitution. Some matplotlib backends invoke `gs` directly.
- **texlive-latex-base** — minimum LaTeX engine (`latex`, `pdflatex`).
- **texlive-latex-extra** — extra packages matplotlib's preambles often
  use (e.g. `\usepackage{amsmath}`).
- **texlive-latex-recommended** — `amsmath`, `babel`, and other commonly-
  used additions.
- **texlive-fonts-recommended** — Latin Modern and other standard math
  fonts.
- **texlive-fonts-extra** — additional fonts that matplotlib may invoke
  (e.g. for `\mathbb`, `\mathcal` variants beyond the default).

### Common matplotlib LaTeX failures

If a user reports "matplotlib LaTeX renders boxes" or "fonts look wrong":

- Boxes / question marks → font package missing. Add `texlive-fonts-extra`.
- Bitmap-looking fonts → cm-super missing or not picked up. Rebuild image,
  run `fc-cache -fv` afterwards.
- `RuntimeError: latex was not able to process` → texlive-latex-extra
  missing for some `\usepackage{...}` matplotlib uses.
- `dvipng was not found` → dvipng missing. This one is obvious from the
  error, but worth checking.

### Verification after build

```bash
docker run --rm myimage python -c "
import matplotlib
matplotlib.use('Agg')
import matplotlib.pyplot as plt
plt.rcParams['text.usetex'] = True
fig, ax = plt.subplots()
ax.set_title(r'\$\\sigma_8(z) = \\int dk\\, P(k,z)\$')
fig.savefig('/tmp/_test.png')
print('usetex render OK')
"
```

If this prints `usetex render OK` without a `RuntimeError`, all needed
